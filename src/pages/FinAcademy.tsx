    ...
    >
      ...
      <div className="flex flex-col md:flex-row items-start md:items-end justify-between space-y-6 md:space-y-0">
        <div>
          ...
          <p className="text-text-secondary text-lg tracking-wide">Financial knowledge becomes financial power.</p>
        </div>

        {/* Header Stats */}
        <div className="flex space-x-4">
          <div className="glass px-6 py-3 rounded-xl border border-white/5 flex flex-col items-center">
            <span className="text-xs text-text-secondary uppercase tracking-widest">Level</span>
          ...
          <div className="glass px-6 py-3 rounded-xl border border-white/5 flex flex-col items-center">
            <span className="text-xs text-text-secondary uppercase tracking-widest">XP Earned</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 mt-12 relative z-10">
        ...
        <div className="lg:col-span-2 game-card p-10 relative overflow-hidden">
          {/* Subtle Background Effects for Roadmap */}
          <div className="absolute top-0 right-0 w-96 h-96 bg-primary/5 rounded-full blur-[100px] pointer-events-none"></div>

          <h2 className="text-2xl font-display font-black text-slate-800 mb-10 flex items-center uppercase tracking-wide">
              ...
              {roadmapNodes.map((node, index) => (
                ...
                >
                  ...
                  <div className={cn(
                    ...
                    node.status === 'completed' ? "bg-slate-50/70 border-slate-200/80 opacity-90" : 
                    node.status === 'current' ? "bg-indigo-50/80 border-primary/40 shadow-md scale-[1.02]" : 
                    "bg-slate-100/40 border-slate-200/40 opacity-55 grayscale"
                  )}>
                    <div className="flex justify-between items-center">
                      <h3 className={cn("font-display font-black text-lg tracking-wide uppercase", 
        ...
        <div className="lg:col-span-1 space-y-8">
          ...
          >
            <Card className="game-card-indigo border-2 border-indigo-200 relative overflow-hidden group">
              <div className="absolute inset-0 bg-gradient-to-br from-indigo-100/50 to-transparent"></div>
              
              <CardHeader className="relative z-10 pb-4">
                <div className="flex items-center space-x-2 mb-4">
                  <Badge variant="outline" className="border-indigo-400 text-indigo-600 bg-indigo-50 uppercase font-bold tracking-wider text-[10px]">Active Quest</Badge>
                  <Badge variant="glass" className="bg-white/80 text-indigo-700 border border-indigo-100 uppercase font-bold tracking-wider text-[10px]">Beginner</Badge>
                </div>
                <CardTitle className="text-2xl font-black text-slate-800 uppercase tracking-wide">Inflation Hunt</CardTitle>
              </CardHeader>
              ...
              <CardContent className="relative z-10 space-y-6">
                 ...
                 <div className="space-y-2">
                   <div className="flex justify-between text-xs font-black uppercase text-indigo-500 tracking-wider">
                     <span>Quest Progress</span>
                     <span>80%</span>
                   </div>
                   <div className="h-3 w-full bg-indigo-100 rounded-full overflow-hidden border border-indigo-200/50">
                      <div className="h-full bg-gradient-to-r from-primary to-indigo-400 w-[80%] rounded-full shadow-[0_2px_4px_rgba(109,93,245,0.3)]"></div>
                   </div>
                 </div>

                 <div className="flex items-center justify-between p-3.5 rounded-xl bg-white border border-indigo-100 shadow-sm">
                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-wider">Reward</span>
        ...
        {missionState !== 'idle' && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 20 }}
              className="w-full max-w-2xl bg-white border-2 border-indigo-200 rounded-3xl overflow-hidden shadow-[0_20px_50px_rgba(15,23,42,0.22)]"
            >
              {/* Intro State */}
              {missionState === 'intro' && (
                <motion.div 
                ...
                >
                  ...
                  <div className="w-20 h-20 bg-indigo-100 rounded-full flex items-center justify-center mb-6 shadow-inner border border-indigo-200 relative z-10">
                    <BookOpen className="w-10 h-10 text-primary" />
                  </div>
                  <h2 className="text-3xl font-display font-black text-slate-800 uppercase tracking-wide mb-4 relative z-10">
              ...
              {missionState === 'scenario' && (
                ...
                >
                  ...
                  <h2 className="text-3xl font-display font-black text-slate-800 mb-4 uppercase tracking-wide">
                  ...
                  <div className="space-y-4">
                    {['Increase savings', 'Diversify investments', 'Hold more cash', 'Ignore the change'].map((opt, i) => (
                      <button
                        ...
                        className={cn(
                          "w-full text-left p-5 rounded-2xl border-2 transition-all duration-300 group flex items-center justify-between shadow-sm",
                          selectedOption === i 
                            ? "bg-indigo-50 border-primary text-primary scale-[1.02]" 
                            : "bg-slate-50 border-slate-200/80 text-slate-700 hover:border-primary/50 hover:bg-indigo-50/40 hover:-translate-y-1 hover:shadow-md"
                        )}
                      >
              ...
              {missionState === 'reward' && (
                <motion.div 
                  ...
                  animate={{ opacity: 1, scale: 1 }}
                  className="p-12 flex flex-col items-center text-center relative overflow-hidden bg-slate-50"
                >
                  ...
                  <div className="w-24 h-24 rounded-full bg-success/15 flex items-center justify-center mb-6 border border-success/30 relative z-10 shadow-sm animate-bounce">
                    <Trophy className="w-10 h-10 text-success" />
                  </div>
                  
                  <h2 className="text-4xl font-display font-black text-slate-800 mb-2 relative z-10 uppercase tracking-wide">Excellent Choice!</h2>
                  <p className="text-slate-500 mb-8 max-w-md relative z-10 text-sm leading-relaxed">
                  ...
                  <div className="flex space-x-6 mb-10 relative z-10">
                    <div className="px-6 py-4 rounded-2xl bg-white border border-slate-200 flex flex-col items-center shadow-sm">
                      <span className="text-[10px] text-slate-400 uppercase tracking-widest font-black mb-1">XP Reward</span>
                      <span className="text-xl font-mono font-bold text-success flex items-center">
                        <Zap className="w-4 h-4 mr-1 text-success animate-pulse" />
                        +250 XP
                      </span>
                    </div>
                    <div className="px-6 py-4 rounded-2xl bg-white border border-slate-200 flex flex-col items-center shadow-sm">
                      <span className="text-[10px] text-slate-400 uppercase tracking-widest font-black mb-1">Unlocked</span>
                      <span className="text-sm font-bold text-primary uppercase tracking-wider">Market Insight</span>
                    </div>
                  </div>
                  <div className="bg-white border-2 border-indigo-100 rounded-3xl p-6 mb-8 w-full max-w-md text-left shadow-sm relative z-10">
                    <div className="flex items-center space-x-2 mb-3">
                      <div className="w-2.5 h-2.5 rounded-full bg-primary animate-pulse"></div>
                      <h4 className="text-primary font-black text-[10px] tracking-[0.2em] uppercase">Reflection</h4>
                    </div>
                    <p className="text-slate-700 text-base font-medium leading-relaxed italic">"What did your decision teach you about changing economic conditions?"</p>
                  </div>
