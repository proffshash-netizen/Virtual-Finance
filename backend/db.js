...
const initDb = async () => {
  return new Promise((resolve, reject) => {
    db.serialize(async () => {
      ...
      db.run(`INSERT OR IGNORE INTO users (id, password, role, display_name) VALUES (?, ?, ?, ?)`,
        ['admin', adminHash, 'admin', 'System Admin']
      );

      // Seed player FIN001
      const player1Hash = await bcrypt.hash('demo123', 10);
      db.run(`INSERT OR IGNORE INTO users (id, password, role, display_name, email, avatar_id, level, xp, money, net_worth, health, streak_days, achievements, unlocked_districts) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
